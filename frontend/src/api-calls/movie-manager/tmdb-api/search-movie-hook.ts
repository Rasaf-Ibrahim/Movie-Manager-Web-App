...
/*__________________________________________
 ...
 ✅ hook 
____________________________________________*/
export function useSearchMovie(search_term) {

    return useQuery({

        queryKey: ["search_movie_hook"],

        queryFn: async () => {

            try {
                const response = await axios_movie_manager_instance.get(`/tmdb/search-movie?search=${search_term}`)
