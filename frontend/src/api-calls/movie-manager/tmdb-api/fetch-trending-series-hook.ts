...
/*__________________________________________
 ...
 ✅ hook 
____________________________________________*/
export function useFetchTrendingSeries(page) {

    return useQuery({

        queryKey: ["fetch_trending_series"],

        queryFn: async () => {

            try {
                const response = await axios_movie_manager_instance.get(`/tmdb/trending-series?page=${page}`)
