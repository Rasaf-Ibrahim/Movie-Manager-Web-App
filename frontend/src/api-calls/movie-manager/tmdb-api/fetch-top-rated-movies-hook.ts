...
/*__________________________________________
 ...
 ✅ hook 
____________________________________________*/
export function useFetchTopRatedMovies(page) {

    return useQuery({

        queryKey: ["fetch_top_rated_movies"],

        queryFn: async () => {

            try {
                const response = await axios_movie_manager_instance.get(`/tmdb/top-rated-movies?page=${page}`)
